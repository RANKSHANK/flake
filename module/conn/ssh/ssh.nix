{ lib, config, user, ... }: let 
    keys = builtins.foldl' (acc: attr: acc // attr) { client = []; builder = []; } (lib.flatten (map (keyPath: lib.pipe keyPath [
            (lib.splitString  "/" )
            (lib.last)
            (lib.splitString "." )
            (strs: let
            ...
            in lib.ternary (name == config.networking.hostName) {} (map (tag: {
                    ${builtins.unsafeDiscardStringContext tag} = [ key ]; 
                }) filtered)
            )
        ])
        (lib.listTargetFilesRecursively ".pub" ./keys)));
in lib.mkModule "ssh" [ "connectivity" ] {
