{
  ...
  inherit (builtins) attrValues listToAttrs readFile replaceStrings;
  inherit (lib.lists) flatten head;
  inherit (lib.modules) mkIf;
  inherit (lib.strings) concatStrings;
  inherit (util) listTargetFilesRecursively mkModule safeIsString;
in
  mkModule "firefox" ["connectivity" "desktop"] {
    home-manager.users.${user} = {
      ...
      programs.firefox = {
        ...
        package = pkgs.wrapFirefox pkgs.firefox.unwrapped {
          nativeMessagingHosts = attrValues {
        ...
        profiles.${user} = let
          ...
          genCSS = dir:
            concatStrings (flatten [
              ...
              (map readFile (listTargetFilesRecursively ".css" dir))
            ]);
        in {
          extensions = {
            force = true;
          ...
          search = {
            force = true;
            engines = listToAttrs (map (entry: {
                name = entry.name;
                value = {
                  urls = [
                    {
                      template = replaceStrings ["{}"] ["{searchTerms}"] entry.url;
                    }
                  ];
                  definedAliases = [
                    "!${entry.shortcut}"
                  ...
                  iconUpdateUrl = mkIf (safeIsString "icon" entry) entry.icon;
                };
              })
              config.browsers.searchEngines);
            default = "${(head config.browsers.searchEngines).name}";
